...
- [x] 本模块所有任务完成。
- [x] 投资组合计算不依赖 React、API Route 或外部网络。
- [x] `/api/prices` 能返回完整快照。
