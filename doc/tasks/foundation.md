...
- [x] FND-005 创建 mock 投资组合快照
  - 创建可供 UI 使用损害 mock `PortfolioSnapshot` 数据。
  - 放在 `lib/mockData.ts` 或等价位置。
  - 验收：首页能读取 mock 数据并显示至少一个字段。

- [x] FND-006 配置基础 npm scripts
  - 添加 `dev`、`build`、`lint`。
  - 如果测试框架已安装，添加 `test`。
  - 验收：`npm run build` 不因项目骨架报错。

- [x] FND-007 创建 `.gitignore`
