...
- [ ] 本模块所有任务完成。
- [ ] `AAPL` 和 `1155.KL` 能获取价格。
- [ ] 汇率能转换为内部 `ExchangeRate`。
- [ ] 第三方响应结构被隔离在 `lib/market/`。
